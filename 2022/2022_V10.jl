...
#
lateCons = @expression(roster, sum(θ[i,k] for i in Intern, k in Week)) # + 100*η[i,k]
...
C = @expression(roster,
    sum( (PubC_1[i,k,d] + PubC_2[i,k,d] + PubD[i,k,d] + SEM[i,k,s] + LATE[i,k] + TIL[i,k,s])
    for i in Intern, k in Week, d in Day, s in WE ))
...
function lateShifts(rost, intern)
    l = convert.(Int64, round.(5*Vector(JuMP.value.(LATE[intern,:]))'))
    [l ; rost]
end
...
function replace_rest(x)
    ...
    rot_name = ["W/E_Clay_1" "W/E_Clay_2" "W/E_Dan" "TIL" "LATE SHIFT"]
...
function indiv_roster(intern, filename, sheetname)
    ...
    rost_with_til = find_til(combined_roster, intern)
