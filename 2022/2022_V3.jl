...
println(JuMP.value.(w[3,:,3,1,:]))


Day = 1:7
Job = 1:8
# 1 = Normal, 2 = late shift, 3 = w/e clay, 4 = w/e dande, 5 = ADO, 6 = Seminar, 7 = Pub Hol, 8 = TIL
@variable(roster, w[Intern, Rotation, Job, Week, Day], Bin )

normal_workdays = @constraint(roster, [i in Intern, k in Week, j in Rotation],
                    sum(w[i,j,1,k,1+alpha] for alpha in 0:6) - 7*x[i,k,j] == 0)

clayton_weekend = @constraint(roster, [j in Rotation, k in Week, l in 6:7],
                    sum(w[i,j,3,k,l] for i in Intern) ==2)

dandenong_weekend = @constraint(roster, [j in Rotation, k in Week, l in 6:7],
                    sum(w[i,j,4,k,l] for i in Intern) ==1)

# need a constraint for 1 shift per weekend

TIL_1 = @constraint(roster, [i in Intern, j in Rotation, k in 1:51, m in 3:4],
                    w[i, j, m, k, 6] - w[i, j,8,k+1,1] == 0)
TIL_2 = @constraint(roster, [i in Intern, j in Rotation, k in 1:51, m in 3:4],
                    w[i, j, m, k, 7] - w[i,j,8,k+1,2] == 0)

@variable(roster, S[Intern, Week], Bin)

late_shift_limiter = @constraint(roster, [k in 5:52], sum(S[i,k] for i in Intern) ==1)

late_shift_week = @constraint(roster, [i in Intern,j in Rotation, k in 5:52],
                    sum(w[i,j,2,k,1+alpha] for alpha in 0:4) - 5*S[i,k] == 0)

# need a late shift constraint for allocating workers at clayton

# need a constraint that equally distributes late shifts and weekend shifts - objective?

@variable(roster, v[Intern,Rotation, Week, Day], Bin)

no_days_off_late_shift = @constraint(roster,
                            [i in Intern,k in Week, l in 1:5, m in [5,8], j in 12:13],
                            S[i,k] + v[i,j,k,l] - 1 <= 0)
no_days_off_late_shift_2 = @constraint(roster,
                            [i in Intern, k in Week, l in 1:5, m in [5,8], j in 12:13],
                            x[i,k,j] + v[i,j,k,l] - 1 <= 0)
no_days_off_late_shift_3 = @constraint(roster, [i in Intern, k in Week, l in 1:5, m in [5,8], j in 12:13],
                            w[i,j,m,k,l] + v[i,j,k,l] <= 0)

twelve_ADOS = @constraint(roster, [i in Intern, j in Rotation],
                sum(w[i,j,5,k,l] for k in 5:52, l in 1:5) ==12)
no_early_ADOS = @constraint(roster, [i in Intern, j in Rotation],
                sum(w[i,j,5,k,l] for k in 1:4, l in 1:5) ==0)
spaced_out_ADOS = @constraint(roster, [i in Intern, j in Rotation, beta in 0:11],
                sum(w[i,j,5,5 + 4*beta + alpha,l] for l in 1:5, alpha in 0:3) ==1)
...
out = sum((i*Matrix(JuMP.value.(x[:,:,i]))) for i in 1:21)
...
XLSX.openxlsx("output/2022_V2_0607_17.20.xlsx", mode="w") do xf
    sheet = xf[1]
    sheet["B1:BA1"] = list
    sheet["B4:BA15"] = df
end
