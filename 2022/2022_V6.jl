...
Weekend_orien_1 = @constraint(roster, [i in Intern], sum(WeC_1[i,k,s] + PubC_1[i,k,d] for k in 1:6, s in WE, d in Day) >= 1)
...
# TIL_ADOs_AL = @constraint(roster, [i in Intern, k in Week],
#             sum(TIL[i,k,s] + ADO[i,k,d] for s in WE, d in Day) <= 1)




# ADOs_AL = @constraint(roster, [i in Intern, k in Week],
#             sum(ADO[i,k,d] + x[i,k,20] for d in Day) <= 1)



# SEM_ADOs = @constraint(roster, [i in Intern, k in Week],
#             sum(SEM[i,k,s] + ADO[i,k,d]+ x[i,k,20] for s in WE, d in Day) <= 1)
...
#                 sum(ADO[i,5 + α + 4*β, d] for d in Day, α in 0:3) >= 1)
